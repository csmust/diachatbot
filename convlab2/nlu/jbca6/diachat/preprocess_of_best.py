...
def preprocess(mode,tokenizerpath,CROSS_TRAIN=False):
    ...
    data = {}
    for key in data_key:
        data[key] = json.load(open(os.path.join(data_dir, "best_"+ key + '.json'),encoding='utf-8'))
        
        print('load best {}, size {}'.format(key, len(data[key])))
                    ...
                    for j in i['slot_values']:
                        if j['value'] is not None and j['value'] != '' and j['value'] != '？' and j['value'] != '?':
                            if j['value'] in utterance:
                                ...
                                # utterance=utterance.replace(j['value'],"^"*len(j['value']),1)
                                span_info.append((
...
if __name__ == '__main__':
  ...
  该参数与config文件的 pretrained_weights参数值保持一致。比如
   ...
   模型训练和加载的时候选用对应的config文件，以便加载对那个的bert模型
    ...
    preprocess('All',path,CROSS_TRAIN=True)
