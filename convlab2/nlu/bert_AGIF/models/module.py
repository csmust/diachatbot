...
class ModelManager(nn.Module):
    ...
    def __init__(self, args, num_slot, num_intent):
        ...
        args.word_embedding_dim = self.bert.config.hidden_size
...
class LSTMDecoder(nn.Module):
    ...
    def __init__(self, args, input_dim, hidden_dim, output_dim, dropout_rate, embedding_dim=None, extra_dim=None):
        ...
        if self.bert_dim is not None:
            self.bert_layer = nn.Embedding(output_dim, embedding_dim)
            self.__init_tensor = nn.Parameter(
                torch.randn(1, self.bert_dim),
                requires_grad=True
            )

        # Make sure the input dimension of iterative LSTM.
        if self.__extra_dim is not None and self.bert_dim is not None:
            lstm_input_dim = self.__input_dim + self.__extra_dim + self.bert_dim
        elif self.__extra_dim is not None:
            lstm_input_dim = self.__input_dim + self.__extra_dim
        elif self.bert_dim is not None:
            lstm_input_dim = self.__input_dim + self.bert_dim
        else:
    ...
    def forward(self, encoded_hiddens, seq_lens, forced_input=None,  # extra_input=None,
        ...
        if self.bert_dim is not None and forced_input is not None:

            forced_tensor = self.bert_layer(forced_input)[:, :-1]
            ...
            for word_i in range(seq_lens[0]):
                ...
                prev_tensor = self.bert_layer(index.squeeze(1)).unsqueeze(1)
