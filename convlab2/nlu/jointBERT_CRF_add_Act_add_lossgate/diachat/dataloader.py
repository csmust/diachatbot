...
class Dataloader:
    ...
    def load_data(self, data, data_key, cut_sen_len, use_bert_tokenizer=True):
        ...
        for d in self.data[data_key]:
            ...
            d.append(self.seq_domain2id(domain_seq))

            act_seq = list(set([x[0] for x in d[3]]))
            d.append(self.seq_act2id(act_seq))
        ...
        if data_key == 'train':
            ...
            for intent, intent_id in self.intent2id.items():
                neg_pos = (
                    train_size - self.intent_weight[intent_id]) / self.intent_weight[intent_id]
            ...
            for domain, domain_id in self.domain2id.items():
                neg_pos = (
                    train_size - self.domain_weight[domain_id]) / self.domain_weight[domain_id]
                self.domain_weight[domain_id] = np.log10(neg_pos)
            self.domain_weight = torch.tensor(self.domain_weight)

            for act, act_id in self.act2id.items():
                neg_pos = (
                    train_size - self.act_weight[act_id]) / self.act_weight[act_id]
                self.act_weight[act_id] = np.log10(neg_pos)
    ...
    def pad_batch(self, batch_data):
        ...
        domain_tensor = torch.zeros(
            (batch_size, self.domain_dim), dtype=torch.float)
        ...
        for i in range(batch_size):
            ...
            tag_mask_tensor[i, 1:sen_len -
                            1] = torch.LongTensor([1] * (sen_len - 2))
            for j in intents:
                intent_tensor[i, j] = 1.
            for j in domains:
                domain_tensor[i, j] = 1.
