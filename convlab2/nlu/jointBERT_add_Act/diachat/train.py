...
from convlab2.nlu.jointBERT_add_Act.diachat.dataloader import Dataloader
from convlab2.nlu.jointBERT_add_Act.diachat.jointBERT_add_Act import JointBERT_add_Act
...
def train(CROSS_TRAIN=False,best_val_F1_list=[],args=None):
    ...
    print('-' * 20 + 'data' + '-' * 20)
    from convlab2.nlu.jointBERT_add_Act.diachat.postprocess import is_slot_da, calculateF1, recover_intent
    ...
    model = JointBERT_add_Act(config['model'], DEVICE, dataloader.tag_dim, dataloader.intent_dim,
    ...
    for step in range(1, max_step + 1):
        ...
        loss = slot_loss + intent_loss  + act_loss
        loss.backward()
        ...
        if step % check_step == 0:
            ...
            for pad_batch, ori_batch, real_batch_size in dataloader.yield_batches(batch_size, data_key='val'):
                ...
                if not config['model']['context']:
                    context_seq_tensor, context_mask_tensor = None, None
                with torch.no_grad():
                    slot_logits, intent_logits,act_logits, slot_loss, intent_loss,act_loss = model.forward(word_seq_tensor,
                                                                                       word_mask_tensor,
                                                                                       tag_seq_tensor,
                                                                                       tag_mask_tensor,
                                                                                       intent_tensor,
