...
from convlab2.nlu.jointBERT_add_Domain.diachat.dataloader import Dataloader
from convlab2.nlu.jointBERT_add_Domain.diachat.jointBERT_add_Domain import JointBERT_add_Domain
...
def train(CROSS_TRAIN=False,best_val_F1_list=[],args=None):
    ...
    print('-' * 20 + 'data' + '-' * 20)
    from convlab2.nlu.jointBERT_add_Domain.diachat.postprocess import is_slot_da, calculateF1, recover_intent
    ...
    model = JointBERT_add_Domain(config['model'], DEVICE, dataloader.tag_dim, dataloader.intent_dim,
    ...
    for step in range(1, max_step + 1):
        ...
        _, _, _, slot_loss, intent_loss, domain_loss = model.forward(word_seq_tensor, word_mask_tensor, tag_seq_tensor, tag_mask_tensor,
        ...
        loss = slot_loss + intent_loss + domain_loss
        loss.backward()
        ...
        if step % check_step == 0:
            ...
            for pad_batch, ori_batch, real_batch_size in dataloader.yield_batches(batch_size, data_key='val'):
                ...
                if not config['model']['context']:
                    context_seq_tensor, context_mask_tensor = None, None
                with torch.no_grad():
                    slot_logits, intent_logits,domain_logits, slot_loss, intent_loss,domain_loss = model.forward(word_seq_tensor,
                                                                                       word_mask_tensor,
                                                                                       tag_seq_tensor,
                                                                                       tag_mask_tensor,
                                                                                       intent_tensor,
