...
from convlab2.nlu.jointBERT_add_Domain_Act.diachat.dataloader import Dataloader
from convlab2.nlu.jointBERT_add_Domain_Act.diachat.jointBERT_add_Domain_Act import JointBERT_add_Domain_Act
...
def train(CROSS_TRAIN=False,best_val_F1_list=[],args=None):
    ...
    print('-' * 20 + 'data' + '-' * 20)
    from convlab2.nlu.jointBERT_add_Domain_Act.diachat.postprocess import is_slot_da, calculateF1, recover_intent
    ...
    domain_vocab = json.load(
        open(os.path.join(data_dir, 'domain_vocab.json'), encoding='utf-8'))
    act_vocab = json.load(
        open(os.path.join(data_dir, 'act_vocab.json'), encoding='utf-8'))
    ...
    print("domain num:", len(domain_vocab))
    ...
    model = JointBERT_add_Domain_Act(config['model'], DEVICE, dataloader.tag_dim, dataloader.intent_dim,
                          dataloader.domain_dim, dataloader.act_dim,
                          dataloader.intent_weight, dataloader.domain_weight, dataloader.act_weight)
    model.to(DEVICE)
    ...
    train_domain_loss, train_act_loss = 0, 0
    best_val_f1 = 0.
    ...
    for step in range(1, max_step + 1):
        ...
        #word_seq_tensor, tag_seq_tensor, intent_tensor, word_mask_tensor, tag_mask_tensor, context_seq_tensor, context_mask_tensor, domain_tensor, act_tensor

        if not config['model']['context']:
            context_seq_tensor, context_mask_tensor = None, None
        _, _, _, _, slot_loss, intent_loss, domain_loss, act_loss = model.forward(word_seq_tensor, word_mask_tensor, tag_seq_tensor, tag_mask_tensor,
        ...
        loss = slot_loss + intent_loss + domain_loss + act_loss
        loss.backward()
        ...
        if step % check_step == 0:
            ...
            train_domain_loss = train_domain_loss / check_step
            print('[%d|%d] step' % (step, max_step))
            print('\t train slot loss:', train_slot_loss)
            print('\t train intent loss:', train_intent_loss)
            print('\t train domain loss:', train_domain_loss)
            ...
            val_domain_loss, val_act_loss = 0, 0
            model.eval()
            for pad_batch, ori_batch, real_batch_size in dataloader.yield_batches(batch_size, data_key='val'):
                ...
                if not config['model']['context']:
                    context_seq_tensor, context_mask_tensor = None, None
                with torch.no_grad():
                    slot_logits, intent_logits,domain_logits,act_logits, slot_loss, intent_loss,domain_loss,act_loss = model.forward(word_seq_tensor,
                                                                                       word_mask_tensor,
                                                                                       tag_seq_tensor,
                                                                                       tag_mask_tensor,
                                                                                       intent_tensor,
                ...
                val_domain_loss += domain_loss.item() * real_batch_size
            ...
            val_domain_loss /= total
            val_act_loss /=total
            print('%d samples val' % total)
            print('\t val slot loss:', val_slot_loss)
            print('\t val intent loss:', val_intent_loss)
            print('\t val domain loss:', val_domain_loss)
            print('\t val act loss:', val_act_loss)

            writer.add_scalar('intent_loss/train',
                              train_intent_loss, global_step=step)
            writer.add_scalar('intent_loss/val',
                              val_intent_loss, global_step=step)

            writer.add_scalar('slot_loss/train',
                              train_slot_loss, global_step=step)
            writer.add_scalar('slot_loss/val', 
                              val_slot_loss, global_step=step)

            writer.add_scalar('domain_loss/train',
                              train_domain_loss, global_step=step)
            writer.add_scalar('domain_loss/val',
                              val_domain_loss, global_step=step)
            ...
            for x in ['intent', 'slot', 'overall']:
                ...
                writer.add_scalar('val_{}/precision'.format(x),
                                  precision, global_step=step)
                writer.add_scalar('val_{}/recall'.format(x),
                                  recall, global_step=step)
    ...
    if CROSS_TRAIN==True:
        if cross_best_f1 < best_val_f1:
            ...
            print(f"第{len(best_val_F1_list)}折 best_val_f1({best_val_f1}) 比历史折大")
