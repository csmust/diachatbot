...
import numpy as np
import torch
from convlab2.nlu.jointBERT_CRF_add_Act.crosswoz.dataloader import Dataloader
from convlab2.nlu.jointBERT_CRF_add_Act.crosswoz.jointBERT_CRF_add_Act import JointBERT_CRF_add_Act
...
if __name__ == '__main__':
    ...
    dataloader = Dataloader(intent_vocab=intent_vocab, tag_vocab=tag_vocab,domain_vocab=domain_vocab,act_vocab=act_vocab,
    ...
    model = JointBERT_CRF_add_Act(config['model'], DEVICE, dataloader.tag_dim, dataloader.intent_dim,dataloader.domain_dim,dataloader.act_dim)
    ...
    for pad_batch, ori_batch, real_batch_size in dataloader.yield_batches(batch_size, data_key=data_key):
        ...
        with torch.no_grad():
            tag_seq_id, intent_logits,act_logits, batch_slot_loss, batch_intent_loss ,batch_act_loss=model.forward(word_seq_tensor,
                                                                                       word_mask_tensor,
                                                                                       tag_seq_tensor,
                                                                                       tag_mask_tensor,
                                                                                       intent_tensor,
