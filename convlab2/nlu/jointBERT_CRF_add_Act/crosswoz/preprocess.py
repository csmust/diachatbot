...
def preprocess(mode):
    ...
    for key in data_key:
        ...
        json.dump(processed_data[key], open(os.path.join(processed_data_dir, '{}_data.json'.format(key)), 'w', encoding='utf-8'),indent=2, ensure_ascii=False)

    print('sentence label num(intent num):', len(all_intent))
    print('tag num:', len(all_tag))
    print('domain num',len(all_domain))
    print('act num',len(all_act))
    print('all intent :',all_intent)
    ...
    json.dump(all_domain, open(os.path.join(processed_data_dir, 'domain_vocab.json'), 'w', encoding='utf-8'), indent=2, ensure_ascii=False)
    json.dump(all_act, open(os.path.join(processed_data_dir, 'act_vocab.json'), 'w', encoding='utf-8'), indent=2, ensure_ascii=False)
