...
class Database(object):
    ...
    def __init__(self):
        ...
        self.data = {}
        db_dir = os.path.abspath(os.path.join(os.path.abspath(__file__),'../../../../data/diachat/database'))
    ...
    def query(self, belief_state, cur_domain):
        ...
        :param belief_state: state['belief_state']   param belief_state:状态(“belief_state”)
        :param cur_domain: maintain by DST, current query domain   cur_domain:按DST维护，当前查询域
        :return: list of entities    实体列表
        """
        if not cur_domain:#如果不存在当前查询领域，返回0
            return []
        cur_query_form = {}
        for slot, value in belief_state[cur_domain].items():
            if not value:
                continue#不存在value，跳过
            if slot == '出发地':
                slot = '起点'
            elif slot == '目的地':
                slot = '终点'
    ...
    def query_schema(self, field, args):
        if not field in self.schema:#field领域：例如餐馆
            raise Exception('Unknown field %s' % field)
        if not isinstance(args, dict):#判断一个对象是否是一个已知的类型
            raise Exception('`args` must be dict')
        db = self.data.get(field)#选择某个领域db
        plan = self.schema[field]#对应schema
        for key, value in args.items():#推荐菜，香椿炒鸡蛋
            if not key in plan:#看key是否在schema里面
                raise Exception('Unknown key %s' % key)
            value_type = plan[key].get('type')#'multiple_in'
            if value_type == 'between':
                ...
                plan[key]['params'] = value#'params'='香椿炒鸡蛋'
        if field in ['地铁', '出租']:
            s = plan['起点']['params']
            e = plan['终点']['params']
            if not s or not e:
        ...
        def func3(item):#item为每一条，第一个是护国寺小吃店，第二个是北京全聚德
            details = item[1]
